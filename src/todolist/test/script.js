...
export class ToDoListClass {
    ...
    static createToDo(container, toDoValue) {
        ...
        trash.classList.add('d-none', 'trash');
    ...
    displayToDos() {
        this.toDoInfo.forEach((task, index) => {
            ToDoListClass.createToDo(task.description, index);
        });
    }

    // update todo
    updateToDo(inputField, newValue) {
        inputField.value = newValue;
    }

    removeSelectedToDo(container, element) {
    ...
    updateTodoStorage(toDoObject, newValue) {
...
export default function listener() {
    todo.addEventListener('keypress', (e) => {
        if (e.key === 'Enter' && todo.value) {
            ...
            ToDoListClass.createToDo(result);
            todo.value = '';
            e.preventDefault();
        }
    });
